...
<Warning>
  You must be the owner of the ER:LC private server in order to access the private server settings necessary for this configuration.
</Warning>
...
<Steps>
  ...
  <Step title="ERLC Command Logs">
    ...
    <Tip>
      Tip: You can confirm that you pasted the webhook URL correctly by running any private server command! If it doesn't show up, then there is an error with the URL entered.
    </Tip>
    <Warning>
      Command logs are managed by ER:LC, not Ducky. If they randomly stop working, it's likely due to one of the following reasons:
  ...
  <Step title="Completed">
    ...
    <Tip>
      Tip: You can run a quick test to ensure everything is working correctly by running the `:log ping` command in-game.
    </Tip>
  </Step>
</Steps>